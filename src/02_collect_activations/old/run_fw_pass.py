...
REASONING_LOG_FILE    = os.path.join(LOG_DIR,    f"reasoning_analysis_{MODEL_SLUG}_{REASONING_DATASET_TAG}.log")
REASONING_OUTPUT_FILE = os.path.join(VECTOR_DIR, f"reasoning_vectors_{MODEL_SLUG}_{REASONING_DATASET_TAG}.pt")
    ...
    with torch.no_grad():
        for i, sample in enumerate(tqdm(dataset, desc="Forward Passes")):
            try:
                ...
                # Will depend on reasoning dataset type, can also modify to have custom start token index
