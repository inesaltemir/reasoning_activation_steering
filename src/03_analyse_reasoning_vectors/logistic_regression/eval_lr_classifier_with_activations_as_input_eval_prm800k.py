...
parser.add_argument(
    ...
    default="/home/ines/Reasoning-activations/results/lr_classifier_math_shepherd/lr_learned_weights.pt",
...
parser.add_argument(
    ...
    default="/home/ines/Reasoning-activations/results/lr_classifier_math_shepherd/lr_classifier_results.json",
...
parser.add_argument(
    "--output_dir", type=str,
    default="/home/ines/Reasoning-activations/results/lr_classifier_math_shepherd/eval_prm800k",
...
# Identical to load_prefix_lookup() in train_lr_classifier_streaming_extended_prm800k.py.
# Loads all prefix shards (each unique problem prefix stored ONCE on disk) into a
# prefix_id -> (acts, meta) lookup. Returns {} for non-PRM800K datasets (no dedup).
# ==========================================
...
# Mirrors run_step_level() in train_lr_classifier_streaming_extended_prm800k.py,
# including the PRM800K prefix-dedup / branch-aware aggregation logic.
...
def aggregate_step_activations(raw_dir: Path, hook_name: str,
                                num_shards: int, d_model: int, index: dict):
    """Stream all shards, accumulate per-step sums.

    PRM800K dedup logic (mirrors train_lr_classifier_streaming_extended_prm800k.py)
    ---------------------------------------------------------------------------
    The on-disk format (when the evaluation dataset is PRM800K, i.e. it has a
    prefix_index / num_prefix_shards in index.pt) stores:
    ...
    For step-level aggregation we therefore want:
      - Each prefix step counted ONCE per problem (not once per branch).
        Key = (sample_idx, step_idx). Prefix data is loaded separately via
        load_prefix_lookup() and added exactly once per prefix_id.
      - Each branch step counted once per branch.
        Key = (sample_idx, branch_idx, step_idx), so different branches of the
        same problem do not collapse into the same row.

    For non-PRM800K datasets (no prefix dedup) the key is simply
    (sample_idx, step_idx), identical to the original behaviour.
    ...
    # ------------------------------------------------------------------
    if has_prefix_dedup:
        print(f"    Loading prefix shards for step aggregation (PRM800K dedup, {hook_name})...")
    ...
    # ------------------------------------------------------------------
    print(f"    Streaming {num_shards} {'branch ' if has_prefix_dedup else ''}shards "
          f"for step aggregation ({hook_name})...")
    ...
    # (sample_idx, step_idx) for non-PRM800K, (sample_idx, branch_idx, step_idx) for PRM800K)
    groups = np.array([k[0] for k in keys], dtype=np.int32)

    n_correct   = int(y.sum())
    n_incorrect = int(len(y) - y.sum())
    dedup_note  = " (PRM800K prefix-dedup, branch-aware keys)" if has_prefix_dedup else ""
    print(f"    Steps: {len(keys):,}  |  Correct: {n_correct:,}  |  Incorrect: {n_incorrect:,}{dedup_note}")
...
def main():
    ...
    has_prefix_dedup = bool(index.get("prefix_index") and index.get("num_prefix_shards", 0) > 0)
    if has_prefix_dedup:
        print(f"  Detected PRM800K prefix-dedup storage format "
              f"({len(index['prefix_index'])} unique prefixes, "
              f"{index['num_prefix_shards']} prefix shards) — "
              f"branch/completion-aware aggregation will be used.")
    ...
    if has_prefix_dedup:
        print("  Note: for PRM800K, per_sample_means in the summary .pt is already "
              "computed per (sample_idx, branch_idx) as prefix+branch combined during "
              "the forward pass, so it correctly aggregates branches/completions "
              "without further dedup logic needed here.")
    ...
    for layer in available_layers:
        ...
        X, y, groups = aggregate_step_activations(raw_dir, hn, num_shards, d_model, index)
