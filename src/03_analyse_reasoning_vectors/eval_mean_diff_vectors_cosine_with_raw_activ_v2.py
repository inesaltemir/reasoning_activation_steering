...
# ---------------------------------------------------------------------------
# Two variants depending on dataset:
#
#   eval_steps_from_raw()         — generic: branch shards only
#   eval_steps_from_raw_prm800k() — PRM800K: branch shards + prefix shards
#
# In PRM800K the raw store splits each problem into:
#   • a shared prefix (problem header + all steps up to the branching point),
#     stored once in  prefix_<hook>/shard_*.pt  /  prefix_meta/shard_*.pt
#   • one branch per alternative completion at the branching step,
#     stored in  <hook>/shard_*.pt  /  meta_shard_*.pt
#
# Evaluating only the branch shards (as the generic function does) misses all
# prefix steps — which are the earlier, overwhelmingly-correct shared steps —
# producing an artificially skewed correct/incorrect ratio.  The PRM800K
# variant loads both sources and builds (sample_idx, step_idx) groups from
# their union, counting each prefix step exactly once.
# ---------------------------------------------------------------------------

def _load_flat_meta_and_acts(raw_dir: Path, safe_name: str,
                              num_shards: int,
                              meta_pattern: str = "meta_shard_{sid:04d}.pt",
                              acts_subdir: str = None) -> tuple[list[dict], list]:
    """
    Load all metadata rows and shard paths for one set of shards.

    Returns:
        all_meta : flat list[dict], one entry per token row
        shard_paths : list[Path] in shard order (for per-layer lazy loading)
    """
    all_meta: list[dict] = []
    shard_paths: list[Path] = []
    acts_root = raw_dir / (acts_subdir or safe_name)

    for sid in range(num_shards):
        meta_path = raw_dir / meta_pattern.format(sid=sid)
        all_meta.extend(torch.load(meta_path, weights_only=False))
        shard_paths.append(acts_root / f"shard_{sid:04d}.pt")

    return all_meta, shard_paths


def _build_step_groups(meta_rows: list[dict],
                        row_offset: int = 0) -> dict[tuple[int, int], list[int]]:
    """
    Group token rows by (sample_idx, step_idx).
    row_offset shifts absolute indices (used to merge branch and prefix groups
    into a single index space).
    Rows with step_idx < 0 are skipped (prompt / padding tokens).
    """
    ...
    for local_idx, meta in enumerate(meta_rows):
        ...
        groups[(sample_idx, step_idx)].append(row_offset + local_idx)
    return groups


def _score_step_groups(step_groups: dict[tuple[int, int], list[int]],
                        all_meta: list[dict],
                        all_acts: torch.Tensor,
                        direction: torch.Tensor,
                        step_scores: dict[str, list]):
    """
    For each (sample_idx, step_idx) group: compute step mean activation,
    determine label by majority vote, compute cosine similarity, append to
    step_scores["correct"] or step_scores["incorrect"].
    Modifies step_scores in-place.
    """
    for (sample_idx, step_idx), row_indices in step_groups.items():
        ...
        step_scores["correct" if step_is_correct else "incorrect"].append(cos_sim)


def eval_steps_from_raw(raw_dir: Path) -> dict[int, dict[str, list]]:
    """
    Generic step-level eval: reads branch shards only.

    Raw shard layout:
        raw_dir/
            index.pt
            meta_shard_0000.pt ...          {sample_idx, step_idx, is_correct, ...}
            blocks.18.hook_out/
                shard_0000.pt ...           (T_tokens, d_model)
    """
    index      = torch.load(raw_dir / "index.pt", weights_only=False)
    num_shards = index["num_shards"]

    print(f"  Loading branch metadata ({num_shards} shards) ...")
    all_meta, shard_paths = _load_flat_meta_and_acts(
        raw_dir, safe_name="",   # not used directly — per-layer below
        num_shards=num_shards,
        meta_pattern="meta_shard_{sid:04d}.pt",
    )
    # shard_paths built above use empty safe_name; rebuild per-layer inside loop
    # Just use all_meta here.
    print(f"  {len(all_meta)} branch token rows")

    step_groups = _build_step_groups(all_meta, row_offset=0)
    print(f"  {len(step_groups)} (sample, step) groups")
    ...
    for layer_int in tqdm(available_layers, desc="  Layers (step eval)"):
        safe_name = layer_to_hook[layer_int].replace(".", "_")
        direction = vectors[layer_int]

        parts = [
            torch.load(raw_dir / safe_name / f"shard_{sid:04d}.pt",
                       weights_only=False).float()
            for sid in range(num_shards)
        ]
        all_acts = torch.cat(parts, dim=0)
        del parts

        _score_step_groups(step_groups, all_meta, all_acts,
                           direction, step_scores[layer_int])
        del all_acts

    _print_step_counts(step_scores)
    return step_scores


def eval_steps_from_raw_prm800k(raw_dir: Path) -> dict[int, dict[str, list]]:
    """
    PRM800K step-level eval: merges branch shards and prefix shards.

    In PRM800K the shared prefix (all steps up to the branching completion) is
    stored once per problem in dedicated prefix shard files.  Branch shards
    only contain the diverging completion token(s) and any steps that follow.
    This function loads both sources and builds one unified (sample_idx, step_idx)
    group set, so every reasoning step — prefix and branch — is evaluated.

    Prefix steps are added only once per problem (they are shared by all
    branches, and the shard stores them once by design), so there is no
    double-counting.

    Shard layout:
        raw_dir/
            index.pt                         — branch index; also contains
                                               prefix_index and num_prefix_shards
            meta_shard_0000.pt ...           — branch token metadata
            prefix_meta/
                shard_0000.pt ...            — prefix token metadata
            blocks.18.hook_out/
                shard_0000.pt ...            — branch activations
            prefix_blocks.18.hook_out/
                shard_0000.pt ...            — prefix activations
    """
    index             = torch.load(raw_dir / "index.pt", weights_only=False)
    num_branch_shards = index["num_shards"]
    num_prefix_shards = index.get("num_prefix_shards", 0)

    if num_prefix_shards == 0:
        print("  WARNING: index.pt reports 0 prefix shards — falling back to branch-only eval.")
        return eval_steps_from_raw(raw_dir)

    # ---- Load branch metadata ----
    print(f"  Loading branch metadata ({num_branch_shards} shards) ...")
    branch_meta: list[dict] = []
    for sid in range(num_branch_shards):
        branch_meta.extend(
            torch.load(raw_dir / f"meta_shard_{sid:04d}.pt", weights_only=False)
        )
    print(f"  {len(branch_meta)} branch token rows")

    # ---- Load prefix metadata ----
    print(f"  Loading prefix metadata ({num_prefix_shards} shards) ...")
    ...
    print(f"  {len(prefix_meta)} prefix token rows")

    # ---- Build step groups from both sources ----
    # Branch rows are indices [0, len(branch_meta))
    # Prefix rows are indices [len(branch_meta), len(branch_meta)+len(prefix_meta))
    # The combined meta list lets _score_step_groups look up any row by index.
    branch_offset = 0
    prefix_offset = len(branch_meta)
    combined_meta = branch_meta + prefix_meta   # flat list; prefix rows offset

    branch_groups = _build_step_groups(branch_meta, row_offset=branch_offset)
    prefix_groups = _build_step_groups(prefix_meta, row_offset=prefix_offset)

    # Merge: prefix steps are keyed by (sample_idx, step_idx) and are unique
    # per problem by construction (stored once).  Branch steps use the same
    # key space so they naturally extend different (sample_idx, step_idx) pairs.
    all_step_groups: dict[tuple[int, int], list[int]] = defaultdict(list)
    for key, rows in prefix_groups.items():
        all_step_groups[key].extend(rows)
    for key, rows in branch_groups.items():
        all_step_groups[key].extend(rows)

    n_prefix_steps = len(prefix_groups)
    n_branch_steps = len(branch_groups)
    n_total_steps  = len(all_step_groups)
    print(f"  {n_prefix_steps} prefix step groups + {n_branch_steps} branch step groups "
          f"= {n_total_steps} total unique (sample, step) groups")

    step_scores: dict[int, dict[str, list]] = {
        l: {"correct": [], "incorrect": []} for l in available_layers
    }

    for layer_int in tqdm(available_layers, desc="  Layers (step eval PRM800K)"):
        hook_name  = layer_to_hook[layer_int]
        safe_name  = hook_name.replace(".", "_")
        direction  = vectors[layer_int]

        # Branch activations: flat [T_branch_total, d_model]
        branch_parts = [
            torch.load(raw_dir / safe_name / f"shard_{sid:04d}.pt",
                       weights_only=False).float()
            for sid in range(num_branch_shards)
        ]
        branch_acts = torch.cat(branch_parts, dim=0)
        del branch_parts

        # Prefix activations: flat [T_prefix_total, d_model]
        prefix_safe = "prefix_" + safe_name
        prefix_parts = [
            torch.load(raw_dir / prefix_safe / f"shard_{sid:04d}.pt",
                       weights_only=False).float()
            for sid in range(num_prefix_shards)
        ]
        prefix_acts = torch.cat(prefix_parts, dim=0)
        del prefix_parts

        # Stack into a single tensor so row indices work uniformly:
        #   combined_acts[0 : len(branch_meta)]         → branch rows
        #   combined_acts[len(branch_meta) :]            → prefix rows
        combined_acts = torch.cat([branch_acts, prefix_acts], dim=0)
        del branch_acts, prefix_acts

        _score_step_groups(all_step_groups, combined_meta, combined_acts,
                           direction, step_scores[layer_int])
        del combined_acts
...
def _print_step_counts(step_scores: dict[int, dict[str, list]]):
    layer0 = available_layers[0]
    n_c = len(step_scores[layer0]["correct"])
    n_i = len(step_scores[layer0]["incorrect"])
    print(f"  Step-level: {n_c} correct steps, {n_i} incorrect steps "
          f"(layer {layer0}; identical count at every layer)")
...
if all_pt:
    ...
    if args.raw_activations_dir is not None:
        ...
        is_prm800k = any("prm800k" in Path(p).name for p in args.eval_datasets)
        if is_prm800k:
            print(f"\n[2/2] Step-level evaluation (PRM800K — branch + prefix shards) from {raw_dir} ...")
            step_scores = eval_steps_from_raw_prm800k(raw_dir)
        else:
            print(f"\n[2/2] Step-level evaluation (branch shards only) from {raw_dir} ...")
...
results_path = os.path.join(args.output_dir, "eval_mean_diff_cosine_prm800k_step_sample_insh_good.json")
